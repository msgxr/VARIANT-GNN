...
def _tokenize_sequences(
    ...
    nuc_t = torch.tensor(tokenize_nucleotides(nuc_seqs), dtype=torch.long).to(device) if nuc_seqs is not None else None
    aa_t = torch.tensor(tokenize_amino_acids(aa_seqs), dtype=torch.long).to(device) if aa_seqs is not None else None
    return nuc_t, aa_t
...
def _make_geo_loader(
    ...
    graphs = [
        preprocessor.row_to_graph(row, label=(int(y[i]) if y is not None else None)) for i, row in enumerate(X_scaled)
    ]
...
def _dnn_eval(model: VariantDNN, loader: TorchDataLoader, device: torch.device) -> Tuple[List[int], List[List[float]]]:
    model.eval()
    preds_all, probs_all = [], []
    with torch.no_grad():
        for batch in loader:
...
class VariantTrainer:
    ...
    def train(
        ...
        with run_ctx:
            ...
            if groups is not None:
                ...
                gss = GroupShuffleSplit(n_splits=1, test_size=cfg.training.test_size, random_state=cfg.seed)
                idx_tr, idx_te = next(gss.split(idx, y, groups))
                logger.info("Hold-out split: GROUP-AWARE by group id (n_groups=%d)", len(np.unique(groups)))
            else:
                idx_tr, idx_te = _tts(idx, test_size=cfg.training.test_size, stratify=y, random_state=cfg.seed)
            ...
            if getattr(getattr(cfg, "training", cfg), "use_label_cleaning", False):
                try:
                    ...
                    _X_tmp, _y_tmp = _preproc_tmp.fit_resample_train(X_train_all, y_train_all)
                    ...
                    logger.info(
                        "ConfidentLearner: %d → %d örnek (%d şüpheli çıkarıldı, tahmini gürültü %%%.1f)",
                        len(_clean_mask),
                        len(_keep),
                        _report.n_flagged,
                        _report.estimated_noise_rate * 100,
                    )
            ...
            fold_results = self._cross_validate(
                X_train_all, y_train_all, nuc_seqs=nuc_tr, aa_seqs=aa_tr, groups=groups_tr, panels=panels_tr
            )
            ...
            logger.info("Cross-validation complete: Binary F1 (Pathogenic §7.3) = %.4f ± %.4f", mean_f1, std_f1)
            ...
            try:
                ...
                logger.error(
                    "Meta-learner fitting beklenmedik hata (%s: %s) — ağırlıklı ortalama kullanılıyor.",
                    type(exc).__name__,
                    exc,
                )
    ...
    def _fit_single(
        ...
        if use_multimodal and nuc_seqs is not None:
            ...
            post_smote_aa = [aa_seqs[i] for i in orig_inner_tr] if aa_seqs else None
        ...
        try:
            ...
            lgbm_model.fit(
                _to_lgbm_frame(X_inner_tr),
                y_inner_tr,
                eval_set=[(_to_lgbm_frame(X_inner_val), y_inner_val)],
                callbacks=[lgb.early_stopping(20, verbose=False), lgb.log_evaluation(-1)],
            )
            logger.info("LightGBM fitted: best_iteration=%d", lgbm_model.best_iteration_)
        except ImportError:
        ...
        gnn_model = self._train_gatv2(
            ...
            y_inner_tr,
            X_val=X_inner_val,
            y_val=y_inner_val,  # ← early stopping now active
            knn_k=knn_k,
            patience=patience,
            nuc_seqs=post_smote_nuc,
            aa_seqs=post_smote_aa,
        )
        ...
        if getattr(cfg.dnn, "use_dann", False) and panels is not None:
            ...
            dnn_model = self._train_dnn_dann(dnn_model, X_proc[:n_orig], y_resampled[:n_orig], p_arr)
        else:
    ...
    def _cross_validate(
        ...
        if len(X) < min_samples_needed:
            logger.warning(
                "Veri cok kucuk (%d ornek) %d-fold CV icin (min %d onerilir). Hata olusabilir.",
                len(X),
        ...
        if groups is not None:
            ...
            skf = StratifiedGroupKFold(n_splits=cfg.training.cv_folds, shuffle=True, random_state=cfg.seed)
            split_iter = skf.split(X, y, groups)
            logger.info("CV: StratifiedGroupKFold (group-aware, n_groups=%d)", len(np.unique(groups)))
        else:
            skf = StratifiedKFold(n_splits=cfg.training.cv_folds, shuffle=True, random_state=cfg.seed)
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(split_iter, start=1):
            ...
            nuc_val_t, aa_val_t = _tokenize_sequences(
                nuc_val if use_multimodal else None, aa_val if use_multimodal else None, self.device
            )
            gnn_preds, gnn_probs_fold = _gatv2_eval(
                gat_model, val_graph, self.device, nuc_ids=nuc_val_t, aa_ids=aa_val_t
            )
            gnn_f1 = float(f1_score(y_val, gnn_preds[: len(y_val)], average="binary", pos_label=1, zero_division=0))
            ...
            if getattr(cfg.dnn, "use_dann", False) and panels is not None:
                ...
                dnn_model = self._train_dnn_dann(dnn_model, X_tr_proc[:n_orig_tr], y_tr_res[:n_orig_tr], p_fold)
            else:
            ...
            dnn_f1 = float(f1_score(y_val, dnn_preds[: len(y_val)], average="binary", pos_label=1, zero_division=0))
        ...
        try:
            ...
            np.savez(
                cfg.paths.reports_dir / "oof_per_model.npz",
                oof=_oof[mask],
                labels=y[mask],
    ...
    def _train_gatv2(
        ...
        for epoch in range(1, cfg.gnn.epochs + 1):
            ...
            loss = _gatv2_epoch(model, train_graph, optimizer, criterion, self.device, nuc_ids=nuc_tr_t, aa_ids=aa_tr_t)

            # Collect checkpoint for SWA
            collected = swa_buffer.push(epoch, cfg.gnn.epochs, model)

            # Train F1 (same graph, eval mode)
            tr_preds, _ = _gatv2_eval(model, train_graph, self.device, nuc_ids=nuc_tr_t, aa_ids=aa_tr_t)
            train_f1 = float(f1_score(y_tr, tr_preds[: len(y_tr)], average="binary", pos_label=1, zero_division=0))

            epoch_entry: dict = {
                "epoch": epoch,
                "loss": round(loss, 6),
                "train_f1": round(train_f1, 4),
                "swa_collected": collected,
            }
            ...
            if val_graph is not None:
                ...
                if epoch % 5 == 0 or epoch == cfg.gnn.epochs:
                    logger.debug(
                        "GATv2 epoch %d/%d | loss=%.4f | train_f1=%.4f | val_f1=%.4f (patience %d/%d | swa=%d)",
                        epoch,
                        cfg.gnn.epochs,
                        loss,
                        train_f1,
        ...
        if swa_buffer.n_collected >= 2:
            ...
            if has_bn:
                ...
                _tmp_loader = _make_geo_loader(preprocessor, X_tr, y_tr, batch_size=min(32, len(X_tr)), shuffle=False)
    ...
    def _train_dnn(
        ...
        cfg = self.cfg
        optimizer = torch.optim.Adam(model.parameters(), lr=cfg.dnn.lr, weight_decay=cfg.dnn.weight_decay)
        ...
        if y_train is not None:
            ...
            try:
                ...
                logger.warning("_train_dnn: y_train=None — class weights DataLoader'dan yeniden hesaplandı.")
            except Exception:
        ...
        for epoch in range(1, cfg.dnn.epochs + 1):
            ...
            if val_loader is not None:
                ...
                if epoch % 5 == 0 or epoch == cfg.dnn.epochs:
                    logger.debug(
                        "DNN epoch %d/%d | loss=%.4f | val_f1=%.4f | swa=%d",
                        epoch,
                        cfg.dnn.epochs,
                        loss,
                        val_f1,
                ...
                if epoch % 5 == 0 or epoch == cfg.dnn.epochs:
                    logger.debug(
                        "DNN epoch %d/%d | loss=%.4f | swa=%d", epoch, cfg.dnn.epochs, loss, dnn_swa.n_collected
                    )
    ...
    def _train_dnn_dann(
        ...
        counts = np.bincount(y, minlength=2).astype(float)
        cw = torch.tensor(counts.sum() / (2.0 * np.clip(counts, 1, None)), dtype=torch.float32).to(dev)
