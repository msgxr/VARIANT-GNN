...
class SampleKNNGraphBuilder:
    ...
    used to connect each node to its k=10 nearest neighbours in feature space.
    Edges with cosine similarity below ``cosine_threshold`` (default 0.3) are
    pruned to remove weak connections.

    Typical usage
    -------------
    >>> builder = SampleKNNGraphBuilder(k=10, cosine_threshold=0.3)
    >>> data = builder.build(X_scaled, y)   # train graph
    >>> test_data = builder.build(X_test)   # inference — fully inductive
    """

    def __init__(self, k: int = 10, cosine_threshold: float = 0.3) -> None:
        self.k = k
    ...
    def build(
        ...
        # ── Prune edges below cosine similarity threshold ────────────
        if self.cosine_threshold > 0:
            mask = cos_sim >= self.cosine_threshold
        ...
        logger.info(
            "SampleKNNGraph: %d nodes, k=%d, %d edges (cosine sim ≥ %.2f)",
            N, k_actual, edge_index.shape[1], self.cosine_threshold,
        )
