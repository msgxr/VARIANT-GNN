...
class FeatureImportanceAnalyzer:
    ...
    def compute_gnn_gradients(
        ...
        try:
            ...
            self._gnn_scores = _gnn_gradient_importance(gnn_model, X, y, edge_index, device)
            ...
            pd.DataFrame(
                {
                    ...
                    "shap_score": self._shap_scores
                    if self._shap_scores is not None
                    else np.zeros(len(self.feature_names)),
                    "gnn_grad_score": self._gnn_scores
                    if self._gnn_scores is not None
                    else np.zeros(len(self.feature_names)),
    ...
    def export_plot(
        ...
        y_pos = range(len(df))
        colors = ["#d62728" if s > df["combined_score"].median() else "#1f77b4" for s in df["combined_score"]]
