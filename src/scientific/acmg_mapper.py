...
_FEAT_IDX: Dict[str, int] = {
    "Ref_Nucleotide": 0, "Alt_Nucleotide": 1, "Codon_Change_Type": 2,
    "AA_Grantham_Score": 3, "GC_Content_Window": 4, "In_CpG_Site": 5,
    "Motif_Disruption_Score": 6, "AA_Polarity_Change": 7,
    "AA_Hydrophobicity_Diff": 8, "AA_Mol_Weight_Diff": 9, "AA_Size_Diff": 10,
    "Protein_Impact_Score": 11, "Delta_Solvent_Accessibility": 12,
    "Secondary_Structure_Disruption": 13, "GERP_RS": 14,
    "PhyloP100way_vertebrate": 15, "phastCons100way_vertebrate": 16,
    "SiPhy_29way_logOdds": 17, "Phylo_Diversity_Index": 18,
    "gnomAD_exomes_AF": 19, "gnomAD_exomes_AF_afr": 20,
    "gnomAD_exomes_AF_eur": 21, "gnomAD_exomes_AF_eas": 22,
    "gnomAD_exomes_AF_sas": 23, "gnomAD_exomes_AF_amr": 24, "ExAC_AF": 25,
    "SIFT_score": 26, "PolyPhen2_HDIV_score": 27, "PolyPhen2_HVAR_score": 28,
    "CADD_phred": 29, "REVEL_score": 30, "MutPred2_score": 31,
    "VEST4_score": 32, "PROVEAN_score": 33, "MutationTaster_score": 34,
    "MetaSVM_score": 35, "MetaLR_score": 36, "MCAP_score": 37,
    "In_Critical_Protein_Domain": 38, "Splice_Site_Distance": 39,
    "Is_Exonic": 40, "Exon_Conservation_Ratio": 41, "OMIM_Disease_Gene": 42,
}
...
def _eval_BA1(x: np.ndarray, shap: np.ndarray) -> ACMGCriterion:
    ...
    return ACMGCriterion(
        "BA1", "Benign", "VeryStrong", met,
        f"gnomAD_AF={af:.5f} {'> 0.05 → populasyonda çok yaygın' if met else '≤ 0.05'}",
        shap_contrib=sc, weight=_STRENGTH_WEIGHTS["Benign"]["VeryStrong"] if met else 0,
    )
...
def _eval_BS1(x: np.ndarray, shap: np.ndarray) -> ACMGCriterion:
    ...
    sc  = _shap_sum(shap, ["gnomAD_exomes_AF", "ExAC_AF"])
    return ACMGCriterion(
        "BS1", "Benign", "Strong", met,
        f"gnomAD_AF={af:.5f} {'0.01–0.05 → hastalık için yüksek' if met else 'eşik dışı'}",
        shap_contrib=sc, weight=_STRENGTH_WEIGHTS["Benign"]["Strong"] if met else 0,
    )
...
def _eval_BS3(x: np.ndarray, shap: np.ndarray) -> ACMGCriterion:
    ...
    return ACMGCriterion(
        "BS3", "Benign", "Strong", met,
        f"MutPred2={mutpred:.3f}, VEST4={vest:.3f} "
        f"{'→ düşük fonksiyonel etki' if met else '→ eşik karşılanmadı'}",
        shap_contrib=sc, weight=_STRENGTH_WEIGHTS["Benign"]["Strong"] if met else 0,
    )
...
def _eval_PM1(x: np.ndarray, shap: np.ndarray) -> ACMGCriterion:
    ...
    return ACMGCriterion(
        "PM1", "Pathogenic", "Moderate", met,
        f"Kritik_Domain={domain:.1f}, Ekzonik={exon:.1f} "
        f"{'→ kritik fonksiyonel bölge' if met else '→ kritik bölgede değil'}",
        shap_contrib=sc, weight=_STRENGTH_WEIGHTS["Pathogenic"]["Moderate"] if met else 0,
    )
...
def _eval_PM2(x: np.ndarray, shap: np.ndarray) -> ACMGCriterion:
    ...
    af  = max(_get(x, "gnomAD_exomes_AF"), _get(x, "ExAC_AF"))
    met = af < 0.0001
    sc  = _shap_sum(shap, ["gnomAD_exomes_AF","ExAC_AF","gnomAD_exomes_AF_afr",
                             "gnomAD_exomes_AF_eur","gnomAD_exomes_AF_eas"])
    return ACMGCriterion(
        "PM2", "Pathogenic", "Moderate", met,
        f"gnomAD_AF={af:.6f} "
        f"{'< 0.0001 → populasyonda yok/nadir' if met else '≥ 0.0001 → populasyonda mevcut'}",
        shap_contrib=sc, weight=_STRENGTH_WEIGHTS["Pathogenic"]["Moderate"] if met else 0,
    )
...
def _eval_PP2(x: np.ndarray, shap: np.ndarray) -> ACMGCriterion:
    ...
    return ACMGCriterion(
        "PP2", "Pathogenic", "Supporting", met,
        f"PolyPhen2={poly:.3f}, PROVEAN={prov:.3f} "
        f"{'→ yüksek etki missense' if met else '→ eşikler karşılanmadı'}",
        shap_contrib=sc, weight=_STRENGTH_WEIGHTS["Pathogenic"]["Supporting"] if met else 0,
    )
...
_ALL_EVALUATORS = [
    _eval_BA1, _eval_BS1, _eval_BS3, _eval_BP4, _eval_BP7,
    _eval_PS3, _eval_PM1, _eval_PM2, _eval_PM4, _eval_PP2, _eval_PP3,
]
...
class ACMGMapper:
    ...
    def classify(
        ...
        shap = np.asarray(shap_values, dtype=float) if shap_values is not None \
        ...
        return {
            ...
            "all_criteria": [
                {
                    "code": c.code, "met": c.met,
                    "category": c.category, "strength": c.strength,
                    "evidence": c.evidence,
                }
                for c in criteria
            ],
    ...
    def classify_batch(
        ...
        shap_m = shap_matrix if shap_matrix is not None \
                 else np.zeros_like(feature_matrix)
        return [
            self.classify(feature_matrix[i], shap_m[i], feature_names)
