...
TOTAL_TEST_EXPECTED = sum(
    v["test_p"] + v["test_b"] for v in PANEL_SPEC.values()
)  # 2000 + 200 + 200 + 60 = 2460
...
class SubmissionValidator:
    ...
    def _check_columns(self, df: pd.DataFrame, report: ValidationReport) -> None:
        ...
        report.checks.append(ValidationCheck(
            ...
            message=(
                f"Tüm {len(self.expected_columns)} kolon mevcut."
                if not missing
                else f"Eksik kolonlar: {missing}"
            ),
            severity="ERROR",
        ))
    ...
    def _check_calibrated_risk(self, df: pd.DataFrame, report: ValidationReport) -> None:
        ...
        report.checks.append(ValidationCheck(
            ...
            message=(
                "calibrated_risk [0, 100] aralığında."
                if out_rng == 0
                else f"{out_rng} satır [0,100] dışında."
            ),
            severity="ERROR",
        ))
    ...
    def _check_confidence(self, df: pd.DataFrame, report: ValidationReport) -> None:
        ...
        report.checks.append(ValidationCheck(
            ...
            passed=int(out_rng) == 0,
            message=(
                "confidence_level [0, 100] aralığında."
                if out_rng == 0
                else f"{out_rng} satır [0,100] dışında."
            ),
            severity="WARNING",
    ...
    def _check_uncertainty(self, df: pd.DataFrame, report: ValidationReport) -> None:
        ...
        report.checks.append(ValidationCheck(
            ...
            passed=int(out_rng) == 0,
            message=(
                "uncertainty_score [0, 1] aralığında."
                if out_rng == 0
                else f"{out_rng} satır [0,1] dışında."
            ),
            severity="WARNING",
        ))
    ...
    def _check_probability_label_consistency(
    ...
    def _check_panel_distribution(
        ...
        for panel, spec in PANEL_SPEC.items():
            ...
            detail = (
                f"{panel}: toplam={total} (P={n_path}, B={n_ben}), "
                f"beklenen={exp_total}"
                + (" ✓" if ok else " ✗ UYUMSUZ")
            )
    ...
    def print_report(report: ValidationReport, verbose: bool = True) -> None:
        ...
        print(f"  Satır : {report.n_rows} "
              f"(Pathogenic={report.n_pathogenic}, Benign={report.n_benign})")
...
def _cli_main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--reference", default=None,
                        help="Kör test referans CSV yolu (panel kontrolü için)")
    parser.add_argument("--output",    default="reports/submission_validation.json",
                        help="JSON rapor çıktı yolu")
    parser.add_argument("--quiet",     action="store_true",
                        help="Yalnızca sonuç satırını göster")
