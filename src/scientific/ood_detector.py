...
class OODDetector:
    ...
    def detect(
        ...
        ood_features = [
            [feat_names[j] for j in range(F) if ood_feat_mask[i, j]]
            for i in range(N)
        ]
    ...
    def drift_report(self, X_new: np.ndarray, feature_names: Optional[List[str]] = None) -> dict:
        ...
        return {
            ...
            "top_drifted_features": [
                {"feature": f, "normalized_shift": round(v, 4)}
                for f, v in top_drifted
            ],
