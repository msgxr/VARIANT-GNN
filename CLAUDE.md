...
**Split:** GROUP-AWARE 80/20 hold-out by Variant_ID (GroupShuffleSplit) + StratifiedGroupKFold 5-Fold, random_state=42. Leakage guard: 0 variants straddle train/test. Karar eşiği: θ=0.6831 — TAM group-aware OOF'ta (n≈3040) KALİBRE + SINIF-DENGELİ (50/50) resample F1-optimal (25x medyan). §3.2 jüri seti DENGELİ olduğundan eşik o prior'a ayarlanır; %74-poz cal'da türetmek dengeli sette -5pp kaybettirir. A→B çapraz-doğrulandı, overfit yok.  
**Results (real TEKNOFEST data — balanced-OOF threshold retrain 2026-06-02, canonical: RESULTS_CANONICAL.json):** ⭐ **YARIŞMA BEKLENTİSİ = balanced_jury_f1=0.8134±0.0103** (§3.2 dengeli 50/50 jüri seti — GERÇEK beklenen skor; eski mis-tuned θ=0.3367 ile 0.7635 olurdu, +5pp kurtarıldı). İç AYRIM gücü (%75-poz hold-out, jüri skoru DEĞİL): Test F1=0.8969, MCC=0.5863, precision=0.8984, recall=0.8953, CV F1=0.8936±0.0004. reports/balanced_jury_f1.json  
**Panel results (test hold-out, group-aware @ θ=0.6831):** General F1=0.8865 | KANSER F1=0.944 | PAH F1=0.9077 | CFTR F1=0.9412  
