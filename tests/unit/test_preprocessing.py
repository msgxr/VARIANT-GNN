...
class TestVariantPreprocessor:
    ...
    def test_row_to_graph(self):
        ...
        pre = VariantPreprocessor(
            use_autoencoder=False, smote_enabled=False, use_acmg_proxy=False
        )  # ACMG proxy kapalı: graph shape testi
        pre.fit_resample_train(X, y)
    ...
    def test_smote_not_applied_on_transform(self):
        ...
        assert X_val_out.shape[0] == n_val, f"transform() SMOTE uyguladı: beklenen {n_val}, alınan {X_val_out.shape[0]}"
