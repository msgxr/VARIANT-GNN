...
class TestEvaluationMetrics:
    ...
    def test_threshold_on_validation_not_test(self):
        ...
        p_cal  = np.clip(y_cal * rng.normal(0.7, 0.2, n) +
                         (1 - y_cal) * rng.normal(0.3, 0.2, n), 0.01, 0.99)
        y_test = rng.integers(0, 2, n)
        p_test = np.clip(y_test * rng.normal(0.7, 0.2, n) +
                         (1 - y_test) * rng.normal(0.3, 0.2, n), 0.01, 0.99)
