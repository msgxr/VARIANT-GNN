...
from src.explainability.group_shap import (  # noqa: E402
    _GROUP_ORDER,
    BIOLOGICAL_GROUPS,
...
ax.annotate(
    f"θ*={best_t_mcc:.2f} (MCC={best_mcc_val:.3f})",
...
ax.annotate(
    "θ=0.8415 (karar eşiği, canonical)",
...
plt.tight_layout()
plt.savefig(FIGS / "17_mcc_threshold_general.png", dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
plt.close()
...
if not _SHAP_OK:
    ...
    for r in ranked:
        ...
        sign = "+" if diff >= 0 else ""
        print(
            f"    {r['label_tr']:35s}: %{r['contribution_pct']:5.1f}  "
    ...
    print("  ✅ shap_group_contributions.json kaydedildi")
    ...
    ax.legend(facecolor="#1f2937", edgecolor="#374151", labelcolor="white", fontsize=10)
    plt.tight_layout()
    plt.savefig(FIGS / "16_shap_group_contributions.png", dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close()
