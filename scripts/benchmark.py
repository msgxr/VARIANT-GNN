...
def _make_df(n: int):
    ...
    return pd.DataFrame({
        ...
        "Panel":      np.random.choice(
            ["General", "Hereditary_Cancer", "PAH", "CFTR"], n
        ),
...
def run_benchmark(sizes: list[int], n_repeats: int = 3) -> list[dict]:
    ...
    results = []
    header  = f"{'N Varyant':>12} | {'Ort. Süre (s)':>14} | {'Min (s)':>10} | {'Max (s)':>10} | {'v/s':>10} | {'v/ms':>8}"
    log.info(header)
    log.info("-" * len(header))
    ...
    for n in sizes:
        ...
        results.append(rec)
        log.info(
            f"{n:>12,} | {mean_t:>14.4f} | {min_t:>10.4f} | "
            f"{max_t:>10.4f} | {vps:>10.1f} | {vpms:>8.4f}"
        )
...
def _plot(results: list[dict], out_path: Path) -> None:
    try:
        ...
        for x, y in zip(ns, vps):
            ax1.annotate(f"{y:,.0f} v/s", (x, y),
                         textcoords="offset points", xytext=(6, 6),
                         fontsize=8, color="#fbd38d")
        ...
        for i, (n, v) in enumerate(zip(ns, lat)):
            ax2.text(i, v + 0.0001, f"{v:.4f} ms",
                     ha="center", va="bottom", fontsize=8, color="#fbd38d")
...
def main():
    ...
    if r10k:
        log.info(
            f"\n🚀  10.000 varyant → {r10k['mean_sec']:.2f} saniye "
            f"({r10k['variants_per_sec']:,.0f} varyant/saniye)"
        )
