...
ax.set_title(
    "Karar Eşiği Analizi — F1 / Precision / Recall Tradeoff\n(Patojenik sınıfı, §7.3)",
    color="white",
    fontsize=11,
    pad=10,
)
...
thresh_data = {
    ...
    "optimal_f1": best_f1,
    "curve": [
        {"threshold": float(t), "f1": float(f), "precision": float(p), "recall": float(r)}
...
ax1.set_title(f"Konfüzyon Matrisi Özeti\n(θ={best_t:.2f}, Test seti n={len(y_te)})", color="white", fontsize=11)
ax1.tick_params(colors="white")
ax1.set_ylabel("Varyant Sayısı", color="white")
...
# ═══════════════════════════════════════════════════════════════════════════════
print("\n── H1: Conformal Prediction ──")
from src.scientific.conformal_prediction import ConformalPredictor  # noqa: E402
...
for alpha, label in [(0.10, "90%"), (0.05, "95%"), (0.01, "99%")]:
    ...
    print(
        f"  {label}: empirical_coverage={report.empirical_coverage:.3f} "
...
print("  JSON     : reports/threshold_analysis.json")
print("  JSON     : reports/conformal_results.json")
