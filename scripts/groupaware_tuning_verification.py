...
import numpy as np, json, warnings, optuna
...
Xdf=ds.features; y=ds.labels.astype(int)
...
Xtr=np.asarray(pre.transform(Xdf.iloc[tr]),dtype=np.float32); ytr=y[tr].astype(int)
Xte=np.asarray(pre.transform(Xdf.iloc[te]),dtype=np.float32); yte=y[te].astype(int)
...
cur=dict(max_depth=6,learning_rate=0.05,n_estimators=200,subsample=0.8,colsample_bytree=0.8,min_child_weight=3,reg_alpha=0.05,reg_lambda=1.0)
cur_cv=cv_f1(cur)
print(f"MEVCUT config group-aware CV F1 = {cur_cv:.4f}")

def obj(t):
    p=dict(max_depth=t.suggest_int('max_depth',3,10),
...
json.dump(dict(cur_cv=cur_cv,ga_cv=st.best_value,delta=st.best_value-cur_cv,best=st.best_params),
          open('/private/tmp/claude-501/-Users-seymanur-Desktop-VARIANT-GNN/3e66a81a-dcd4-4709-9460-3921b6d773ae/scratchpad/ga_tune_result.json','w'),indent=2)
