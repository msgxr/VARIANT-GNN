...
# HELPERS
# ─────────────────────────────────────────────
# NOTE: Do NOT call st.cache_resource.clear() at module level —
# it defeats the purpose of @st.cache_resource and causes the model
# to reload on every Streamlit re-run (every click).
...
def main():
    ...
    with tab_about:
        ...
        with col_m4:
            ...
            <div class="model-card">
                <h4>🤖 DNN</h4>
                <p>BatchNorm + Dropout ile çok katmanlı sinir ağı [input→128→64→2] + Stacking Meta-Learner<br>
  ...
  └────┬──────────┬───────────┬──────────┬─────────────────┘
   ...
   (w=0.35)   (w=0.30)   (w=0.25)   (w=0.10)
       ...
       └──────────┴───────────┴──────────┘
        ...
        with col_g2:
            st.info(
                "Bu repo private olduğu için erişim için "
                "GitHub hesabınızla davet almanız gerekmektedir."
            )
